...
static bool _get_help_section_text(int section, formatted_string &text, map<int, int> &hotkeys)
{
    switch (section)
    {
        case '?':
            if (crawl_state.game_is_hints_tutorial())
                text = _col_conv(_add_formatted_hints_help);
            else
                text = _col_conv(_add_formatted_keyhelp);
            return true;
        case CK_HOME:
            text = _col_conv(_add_formatted_help_menu);
            return true;
        default:
            break;
    }
    ...
    {
        if (help_files[i].hotkey != section)
            continue;
        ...
        while (fgets(buf, sizeof buf, fp))
        {
            ...
            if (next_is_hotkey && (isaupper(buf[0]) || isadigit(buf[0]))
                || is_first && section == '*')
            {
                int hotkey = (is_first ? section : tolower(buf[0]));
...
void show_help(int section, string highlight_string)
{
    ...
    shared_ptr<UIText> title = make_shared<UIText>("<yellow>Dungeon Crawl Help</yellow>");
    ...
    bool done = false;
    int last_key, current_page = 0;

    map<int, int> hotkeys;
    formatted_string dummy;
    _get_help_section_text('*', dummy, hotkeys);
    hotkeys.erase('*');
    ...
    popup->on(UI::slots.event, [&](wm_event ev) {
        ...
        {
            scroll = (hotkeys[key]-2) * line_height;
            key = '*';
        }

        formatted_string new_text;
        switch (key)
        {
            case CK_ESCAPE: case ':': case '#': case '/': case 'q': case 'v':
                ...
                break;
            default:
                bool ok = (key == '*' && current_page == '*') || _get_help_section_text(key, new_text, hotkeys);
                if (!ok)
                    break;
                if (key != current_page)
